...
describe("fileTree", function () {
    var traverseTreeAndFindNode = function (inputNode, nodes) {
        ...
        for (var i = 0; i < nodes.length; i++) {
            ...
            var nodeFoundAtChildren = traverseTreeAndFindNode(
                inputNode,
                node.getChildren()
            );
    ...
    function testTreeNode(inputNode, foundNode) {
        it(
            "tree node " + inputNode.id + " should have correct data",
            function () {
        ...
        it(
            "tree node " + inputNode.id + " should have correct parent",
            function () {
                if (inputNode.parentId) {
                    expect(foundNode.parentNode).not.toBeNull();
                    expect(foundNode.parentNode.nodeId).toEqual(
                        inputNode.parentId
                    );
                } else {
    ...
    function testTreeContentsWithDataSet(dataSet) {
        describe("created from " + dataSet + " dataSet", function () {
            ...
            for (var i = 0; i < inputData.length; i++) {
                ...
                var foundNode = traverseTreeAndFindNode(
                    inputNode,
